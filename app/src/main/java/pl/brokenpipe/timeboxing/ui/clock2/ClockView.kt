...
import pl.brokenpipe.timeboxing.ui.clock2.ClockSpinSide.LEFT
 ...
 */
@BindingMethods(
    BindingMethod(type = ClockView::class, attribute = "angle", method = "setAngle"),
    BindingMethod(type = ClockView::class, attribute = "angleAttrChanged",
                  method = "setAngleAttrChangedInverseBindingListener"),
...
@InverseBindingMethods(
    InverseBindingMethod(type = ClockView::class, attribute = "angle", method = "getAngle"),
...
class ClockView(context: Context, attributeSet: AttributeSet):
    ...
    var angleAttrChangedInverseBindingListener: InverseBindingListener? = null
    ...
    var currentSpinSide: ClockSpinSide = LEFT
        set(value) {
            field = value
    ...
    override fun onTouch(view: View, event: MotionEvent): Boolean {
        when (event.action) {
            MotionEvent.ACTION_UP -> {
                ...
                angleAttrChangedInverseBindingListener?.onChange()
                isClockHandDragged = false
                onClockFaceTouchListener?.onHandleDragStop( Math.abs(this.angle) + 360 * fullSpinsCount)
                return true
            }
            MotionEvent.ACTION_DOWN -> {
                ...
                if (isClockHandDragged) {
                    onClockFaceTouchListener?.onHandleDragStart( Math.abs(angle) + 360 * fullSpinsCount)
                }
                return true
            }
            MotionEvent.ACTION_MOVE -> {
                ...
                if (isClockHandDragged) {
                    ...
                    angleAttrChangedInverseBindingListener?.onChange()
                    onClockFaceTouchListener?.onHandleDragging(
                        Math.abs(angle) + 360 * fullSpinsCount)
                }
                return true
            }
        }
...
//        }
    ...
    fun angleChanges() = angleChangeSubject.asObservable()
